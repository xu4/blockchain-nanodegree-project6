App = {
    ...
    initSupplyChain: function () {
        ...
        $.getJSON(jsonSupplyChain, function(data) {
            ...
            App.contracts.SupplyChain.deployed().then(async function(instance) {
                var ownerId = await instance.owner();
    ...
    handleButtonClick: async function(event) {
        ...
        switch(processId) {
            ...
            case 10:
                ...
                break;
            case 11:
                return await App.setupRoles(event);
                break;
            }
    },

    setupRoles: function(event) {
        event.preventDefault();
        var processId = parseInt($(event.target).data('id'));
        App.readForm();
        ...
        App.contracts.SupplyChain.deployed().then(async function(instance) {
            ...
            const farmerBool  = await instance.isFarmer(App.originFarmerID);
            const distributorBool  = await instance.isDistributor(App.distributorID);
            const retailerBool  = await instance.isRetailer(App.retailerID);
            const consumerBool  = await instance.isConsumer(App.consumerID);
       
            var result =  App.originFarmerID+' isFarmer?'+ farmerBool+', '+ App.distributorID+' isDistributor?'+ distributorBool+', '
            result += App.retailerID+' isRetailer?'+ retailerBool+', '+ App.consumerID+' isConsumer?'+ consumerBool;

            return result;
        }).then(function(result) {

            $("#ftc-item").text(result);
            console.log('setupRoles:',result);
        }).catch(function(err) {
            console.log(err.message);
        });
    },
    ...
    harvestItem: function(event) {
        ...
        App.contracts.SupplyChain.deployed().then(function(instance) {
            ...
            console.log('harvestItem: fromAddress:' + App.originFarmerID );
